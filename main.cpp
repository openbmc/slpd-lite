                          ...
                          void* /*userdata*/)
{
    ...
    // This code current assume a maximum of 255 bytes in a recieve
    // or respone message. Enforce that here.
    if (recvBuff.size() > slp::header::MAX_LEN)
    {
        std::cout << "Message size exceeds maximum allowed: " << recvBuff.size()
