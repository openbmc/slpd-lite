...
slp::buffer prepareHeader(const slp::Message& req)
{
    ...
    auto langtagLen = endian::to_network(req.header.langtagLen);
...
std::tuple<int, slp::buffer> processSrvTypeRequest(
...
std::tuple<int, slp::buffer> processSrvRequest(
    const slp::Message& req)
{
...
std::tuple<int, slp::buffer> processRequest(
...
slp::buffer processError(const slp::Message& req,
                         uint8_t err)
{
    slp::buffer buff;
