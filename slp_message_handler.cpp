...
slp::buffer slp::Handler::prepareHeader(const slp::Message& req)
{
    ...
    slp::buffer buff(length, 0);

    buff[slp::header::VERSION_OFFSET] = req.header.version;
    ...
    std::copy_n(&length, slp::header::SIZE_LENGTH,
                buff.data() +
                slp::header::LENGTH_OFFSET);

    auto flags = endian::to_network<uint16_t>(req.header.flags);
    ...
    auto xid = endian::to_network<uint16_t>(req.header.xid);
    std::copy_n((uint8_t*)&xid, slp::header::SIZE_XID,
                buff.data() +
                slp::header::XID_OFFSET);

    auto langtagLen = endian::to_network<uint16_t>(req.header.langtagLen);
    std::copy_n((uint8_t*)&langtagLen, slp::header::SIZE_LANG,
                buff.data() +
                slp::header::LANG_LEN_OFFSET);

    std::copy_n((uint8_t*)req.header.langtag.c_str(), req.header.langtagLen,
                buff.data() +
    ...
    const slp::Message& req)
{
    ...
    */
    ...
    ServiceList svcList = readSLPServiceInfo(slp::CONF_FILE);
    if (svcList.size() <= 0)
    {
        buff.resize(0);
    ...
    const slp::Message& req)
{
    ...
    */
    ...
    for (auto & addr : ifaddrList)
    {
        ...
        uint8_t reserved = 0;
        uint16_t auth = 0;
        uint16_t lifetime = endian::to_network<uint16_t>
                            (slp::LIFETIME);
...
std::list<std::string> slp::Handler::getIntfAddrs()
{
    ...
    for (ifa = ifaddrPtr.get(); ifa != nullptr; ifa = ifa->ifa_next)
    {
        // walk interfaces
    ...
    //NOTE:-no need to delete the ifa
    //print the list
    ...
    const slp::Message& msg)
{
    ...
    {
        case (uint8_t)FunctionType::SRVTYPERQST:
            std::tie(rc, resp) = Handler::processSrvTypeRequest(msg);
            break;
        case (uint8_t)FunctionType::SRVRQST:
...
slp::Handler::ServiceList  slp::Handler::readSLPServiceInfo(
