...
class Parser
{
    private:
        ...
        /** Parse a service request.
          *
          * @param[in] buffer - The buffer from which data should be parsed.
          * @param[out] msg - The msg object into which buffer should be parsed.
          *
          * @return Zero on success, or a non-zero error code.
          *
          * @internal
        ...
        static uint8_t parseSrvRqst(const std::vector<uint8_t>& buffer,
                                    SLP::Message& msg);
...
class Handler
{
    ...
    private:
        ...
        static uint8_t processSrvRequest(const Message& msg,
                                         std::vector<uint8_t>& buff);
        ...
        static uint8_t getIntfAddrs(std::list<std::string>& addrList);
