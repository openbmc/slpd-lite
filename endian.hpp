...
struct convert
{
    static T to_host(T) = delete;
    static T from_host(T) = delete;
...
{
    static uint16_t to_host(uint16_t i) { return htole16(i); };
    static uint16_t from_host(uint16_t i) { return le16toh(i); };
...
{
    static uint32_t to_host(uint32_t i) { return htole32(i); };
    static uint32_t from_host(uint32_t i) { return le32toh(i); };
    static uint32_t to_network(uint32_t i) { return htobe32(i); };
    static uint32_t from_network(uint32_t i) { return be32toh(i); };
};
}

template<typename T> T to_host(T i)
{
    return details::convert<T>::to_host(i);
}

template<typename T> T from_host(T i)
{
    return details::convert<T>::from_host(i);
}
