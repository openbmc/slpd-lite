                                      ...
                                      Message& req)
{
    ...
    if (buffer.size() < SLP::REQUEST::SRVTYPE_LEN)
        return (uint8_t)SLP::Error::PARSE_ERROR;

    /* Parse the PRList. */
    memcpy(&(req.body.srvtyperqst.prlistlen),
           buffer.data() + SLP::REQUEST::ERROR_OFFSET,
           SLP::REQUEST::SIZE_SERVICE);
    ...
    uint8_t pos = SLP::REQUEST::SERVICELEN_OFFSET +
                  req.body.srvtyperqst.prlistlen;
